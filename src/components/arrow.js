...
export default class Arrow {
  ...
  calculatePath() {
    ...
    const endY = this.gantt.options.headerHeight
      ...
      + (this.gantt.options.padding + this.gantt.options.barHeight)
        * this.toTask.task.index
      + this.gantt.options.padding;

    const fromIsBelowTo = this.fromTask.task.index > this.toTask.task.index;
    ...
    if (
      ...
      const down2 = this.toTask.$bar.getY() + this.toTask.$bar.getHeight() / 2 - curveY;
